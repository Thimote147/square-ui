...
const GITHUB_BASE_URL =
  'https://raw.githubusercontent.com/Thimote147/square-ui/master/templates/'
  ...
  try {
    ...
    content.files.forEach((file, index) => {
      ...
      try {
        ...
        if (!fs.existsSync(localPath)) {
          ...
          if (verbose) {
            console.log(
              `    ⚠️  Fichier introuvable: ${path.relative(process.cwd(), localPath)}`,
            )
          }
          return
        }
        ...
        if (file.content && !forceUpdate) {
          ...
          if (verbose) {
            console.log(
              `    ✓ Content déjà présent: ${path.basename(localPath)} (${formatFileSize(fileStats.size)})`,
            )
          }
          return
        }
        ...
        if (dryRun) {
          console.log(
            `    🔄 [DRY RUN] Ajouterait content pour: ${path.basename(localPath)}`,
          )
        } else {
          ...
          if (verbose) {
            console.log(
              `    ✓ Content ajouté: ${path.basename(localPath)} (${formatFileSize(fileStats.size)})`,
            )
          }
        }
      } catch (error) {
        ...
        if (verbose) {
          console.log(
            `    ❌ Erreur lecture: ${path.basename(localPath)} - ${error.message}`,
          )
        }
      }
    })
    ...
    if (modified && !dryRun) {
      try {
        ...
        console.log(
          `  ✅ Mis à jour: ${jsonFile} (${filesAdded} fichier${filesAdded > 1 ? 's' : ''} ajouté${filesAdded > 1 ? 's' : ''})`,
        )
      } catch (error) {
        stats.jsonFiles.errors++
        stats.errors.push({
...
console.log(
  `📂 ${jsonFiles.length} fichier${jsonFiles.length > 1 ? 's' : ''} JSON trouvé${jsonFiles.length > 1 ? 's' : ''}\n`,
)
